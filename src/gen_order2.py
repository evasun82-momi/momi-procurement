...
PRODUCTS = [
  ...
  {"id":"MCCA64","name":"摩米營養護極幼草粉-原味64克","cat":"飼料","stock":438},
  {"id":"MCCATB","name":"摩米草粉試吃禮-原味","cat":"飼料","stock":-1132},
  {"id":"MCCATB-S","name":"摩米草粉自用(單條)-原味","cat":"飼料","stock":485},
  {"id":"MCCB64","name":"摩米營養護極幼草粉-香蕉味64克","cat":"飼料","stock":-10},
  {"id":"MCCBB100","name":"摩米保康全營養草粉-香蕉*100條","cat":"飼料","stock":-3},
  ...
  {"id":"TIMCA1","name":"摩米營養全幼兔A 1公斤","cat":"飼料","stock":218},
  {"id":"TIMCA5","name":"摩米營養全幼兔A 5公斤","cat":"飼料","stock":37},
  {"id":"TIMCG1","name":"摩米營養全天竺鼠G 1公斤","cat":"飼料","stock":887},
  {"id":"TIMCG5","name":"摩米營養全天竺鼠G 5公斤","cat":"飼料","stock":39},
  {"id":"TIMCT1","name":"摩米營養全成兔T 1公斤","cat":"飼料","stock":1223},
  {"id":"TIMCT5","name":"摩米營養全成兔T 5公斤","cat":"飼料","stock":214},
  {"id":"TIMIC1","name":"摩米營養全老兔IC 1公斤","cat":"飼料","stock":168},
  {"id":"TIMIC5","name":"摩米營養全老兔IC 5公斤","cat":"飼料","stock":47},
  {"id":"TIMNT4","name":"摩米全天然T 4磅裝","cat":"飼料","stock":6},
  ...
  {"id":"MOMI-FIRST","name":"First Cut第一割提摩西草","cat":"牧草","stock":-131},
  {"id":"MOMI-SECOND","name":"Second Cut第二割提摩西草","cat":"牧草","stock":-2217},
  ...
  {"id":"TIMF05","name":"第一割提摩西草500G","cat":"牧草","stock":25},
  {"id":"TIMF1","name":"第一割提摩西草1000G","cat":"牧草","stock":277},
  {"id":"TIMF10","name":"第一割提摩西草10KG","cat":"牧草","stock":-37},
  {"id":"TIMF25","name":"第一割提摩西草2.5KG","cat":"牧草","stock":236},
  {"id":"TIMF5","name":"第一割提摩西草5KG","cat":"牧草","stock":-12},
  {"id":"TIMFTB-10","name":"一割試吃包*10","cat":"牧草","stock":-30},
  {"id":"TIMS","name":"Second Cut第二割提牧草(未壓)","cat":"牧草","stock":-503},
  {"id":"TIMS05","name":"第二割提摩西草500G","cat":"牧草","stock":60},
  {"id":"TIMS1","name":"第二割提摩西草1000G","cat":"牧草","stock":204},
  {"id":"TIMS10","name":"第二割提摩西草10KG","cat":"牧草","stock":-12},
  {"id":"TIMS25","name":"第二割提摩西草2.5KG","cat":"牧草","stock":206},
  {"id":"TIMS5","name":"第二割提摩西草5KG","cat":"牧草","stock":-7},
...
SALES = {
  ...
  "PTIMSTB-10":[0,2,0],"PTS2":[97,26,8],"TIMF":[0,0,15],
...
def calc(p):
    ...
    daily = avg/30
    stock = p["stock"]
    days_left = round(stock/daily,1) if daily>0 else 9999
    # order enough to have TARGET days of stock at arrival, plus SAFETY buffer
    order = max(0, math.ceil((LEAD+TARGET+SAFETY)*daily - stock))
    st = "⚠️緊急" if days_left<45 and daily>0 else ("⚠留意" if days_left<90 and daily>0 else "✅充足")
    if daily==0: st="—"
    return round(avg,1), round(daily,2), days_left if daily>0 else "—", st, order

wb = Workbook()
thin = Side(style="thin",color="E2E8F0")
bdr  = Border(left=thin,right=thin,top=thin,bottom=thin)

HDR_BG={"美麗多":"8B5CF6","飼料":"F59E0B","牧草":"22C55E"}
DANGER="FEE2E2"; WARN="FEF3C7"; ALT="F8FAFC"

cats=["美麗多","飼料","牧草"]
for ci,cat in enumerate(cats):
    ws = wb.active if ci==0 else wb.create_sheet(cat)
    if ci==0: ws.title=cat
    prods=[p for p in PRODUCTS if p["cat"]==cat]

    hdrs=["品號","品名","即時庫存","月均銷量","日均","庫存天數","狀態","建議訂購量"]
    ws.row_dimensions[1].height=30
    for c,h in enumerate(hdrs,1):
        cell=ws.cell(row=1,column=c,value=h)
        cell.font=Font(name="Arial",bold=True,color="FFFFFF",size=10)
        cell.fill=PatternFill("solid",start_color=HDR_BG[cat])
    ...
    for ri,p in enumerate(prods,2):
        avg,daily,days_left,st,order = calc(p)
        row=[p["id"],p["name"],p["stock"],avg,daily,days_left,st,order]
        if isinstance(days_left,float):
            bg=DANGER if days_left<45 else (WARN if days_left<90 else (ALT if ri%2==0 else "FFFFFF"))
        else:
            bg=ALT if ri%2==0 else "FFFFFF"
        for c,val in enumerate(row,1):
            cell=ws.cell(row=ri,column=c,value=val)
            cell.font=Font(name="Arial",size=10,
                           bold=(c==8 and isinstance(val,int) and val>0),
                           color=("B45309" if c==8 and isinstance(val,int) and val>0 else "000000"))
