...
const INIT_SALES = {
  ...
  MLMP07:  [0,   19,  53,  56,  34,  136, 72,  134, 111, 109],
...
const INIT_SHIPMENTS = [
  { id:0, label:"TWMIL26/06", shipDate:"2026-05-01", arrivalDate:"2026-07-31", status:"past", note:"已出貨，預計7/31到港",
    allocs:{MLMP2:960,MLMS2:840,MLMP4:400,MLMS07:30,MLMP07:90,MLMS10:20,MLMP10:40,MMC5:0,MMCP5:0,MLMN005:0,MMR10L:0}},
  { id:1, label:"TWMIL26/07", shipDate:"2026-06-08", arrivalDate:"2026-09-15", status:"past", note:"已出貨，預計9/15到港",
    allocs:{MLMP2:1200,MLMS2:720,MLMP4:320,MLMS07:60,MLMP07:60,MLMS10:20,MLMP10:40,MMC5:0,MMCP5:0,MLMN005:0,MMR10L:0}},
  { id:2, label:"TWMIL26/08", shipDate:"2026-08-02", arrivalDate:"2026-10-31", status:"planned", note:"✅ 避開中秋後、國慶前",
    allocs:{MLMP2:3666,MLMS2:2538,MLMP4:280,MLMS07:0,MLMP07:0,MLMS10:0,MLMP10:0,MMC5:0,MMCP5:0,MLMN005:0,MMR10L:0}},
  { id:3, label:"TWMIL26/09", shipDate:"2026-09-12", arrivalDate:"2026-12-15", status:"planned", note:"✅ 避開聖誕前兩週",
    allocs:{MLMP2:2679,MLMS2:1833,MLMP4:490,MLMS07:240,MLMP07:120,MLMS10:39,MLMP10:78,MMC5:0,MMCP5:0,MLMN005:0,MMR10L:0}},
  { id:4, label:"TWMIL26/10", shipDate:"2026-10-17", arrivalDate:"2027-01-15", status:"planned", note:"⚠️ 年前必到！春節1/29，最晚1/15到港",
    ...
    <div style={{fontFamily:"'Noto Sans TC',system-ui,sans-serif",background:"#f1f5f9",minHeight:"100vh"}}>
      <div style={{background:"#1e293b",padding:"14px 18px"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          <button onClick={()=>setImportStep("stock")} style={{background:"#334155",color:"#e2e8f0",border:"none",borderRadius:8,padding:"8px 14px",fontSize:13,cursor:"pointer"}}>📥 匯入庫存</button>
          <button onClick={exportOrder} style={{background:"#3b82f6",color:"#fff",border:"none",borderRadius:8,padding:"8px 14px",fontSize:13,cursor:"pointer",fontWeight:700}}>📤 匯出訂單</button>
        </div>
      </div>
      ...
      {importStep && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
          <div style={{background:"#fff",borderRadius:16,padding:28,width:"100%",maxWidth:380}}>
            ...
            <button onClick={()=>setImportStep(null)} style={{background:"#f1f5f9",border:"none",borderRadius:8,padding:"10px 20px",cursor:"pointer"}}>取消</button>
          </div>
        </div>
      )}

      <div style={{background:"#fff",borderBottom:"1px solid #e2e8f0",display:"flex"}}>
        {[["schedule","船期總覽"],["alloc","分配訂貨"],["suggest","庫存狀況"],["products","品項"],["settings","參數"]].map(([key,label])=>(
          <button key={key} onClick={()=>setActiveTab(key)} style={{flex:1,background:"none",border:"none",borderBottom:activeTab===key?"3px solid #3b82f6":"3px solid transparent",color:activeTab===key?"#3b82f6":"#64748b",padding:"11px 2px",fontSize:12,cursor:"pointer",fontWeight:activeTab===key?700:400}}>{label}</button>
        ))}
      </div>
      ...
      <div style={{padding:"16px 16px 40px",maxWidth:640,margin:"0 auto"}}>
        {activeTab==="schedule" && (
          ...
          <Card style={{background:"#fffbeb",borderColor:"#fde68a"}}>
            ...
            <div style={{fontSize:11,color:"#78350f",lineHeight:1.7}}>🇪🇪 愛沙尼亞：仲夏節、聖誕、新年、復活節<br/>🇹🇼 台灣：春節、228、清明、端午、中秋、國慶</div>
          </Card>
          {shipments.map((sh,si)=>{
              ...
              <Card key={sh.id} borderColor={sh.status==="past"?"#e2e8f0":isOver?"#fecaca":sh.status==="planned"?"#bbf7d0":"#e2e8f0"}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                  ...
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:18,fontWeight:800,color:isOver?"#ef4444":sh.status==="past"?"#94a3b8":"#1e293b"}}>{pals.toFixed(1)}<span style={{fontSize:12,fontWeight:400,color:"#94a3b8"}}> / {CONTAINER_PALLETS}板</span></div>
                    <div style={{fontSize:11,color:isOver?"#ef4444":"#22c55e",fontWeight:600}}>{isOver?`超出${(pals-CONTAINER_PALLETS).toFixed(1)}板`:`剩${(CONTAINER_PALLETS-pals).toFixed(1)}板`}</div>
                  </div>
                </div>
                {pals>0&&(<div style={{background:"#f1f5f9",borderRadius:99,height:8,marginBottom:12,overflow:"hidden"}}><div style={{height:"100%",borderRadius:99,width:`${pct}%`,background:sh.status==="past"?"#cbd5e1":isOver?"#ef4444":pct>85?"#f59e0b":"#3b82f6"}}/></div>)}
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:8}}>
                  <div><Label>出貨日</Label>{sh.status==="past"?<div style={{fontSize:14,fontWeight:600,color:"#94a3b8"}}>{sh.shipDate}</div>:<input type="date" value={sh.shipDate} onChange={e=>updateShipment(sh.id,"shipDate",e.target.value)} style={{width:"100%",border:"1px solid #e2e8f0",borderRadius:8,padding:"7px 10px",fontSize:13,boxSizing:"border-box"}}/>}</div>
                  <div><Label>預計到港</Label>{sh.status==="past"?<div style={{fontSize:14,fontWeight:600,color:"#94a3b8"}}>{sh.arrivalDate}</div>:<input type="date" value={sh.arrivalDate} onChange={e=>updateShipment(sh.id,"arrivalDate",e.target.value)} style={{width:"100%",border:"1px solid #e2e8f0",borderRadius:8,padding:"7px 10px",fontSize:13,boxSizing:"border-box"}}/>}</div>
                </div>
                {pals>0&&(<div style={{background:"#f8fafc",borderRadius:10,padding:"10px 12px",fontSize:12}}>{products.filter(p=>Number(sh.allocs[p.id]||0)>0).map(p=>{const qty=Number(sh.allocs[p.id]);return(<div key={p.id} style={{display:"flex",justifyContent:"space-between",marginBottom:3,color:sh.status==="past"?"#94a3b8":"#475569"}}><span>{p.name.replace("美麗多木質墊料","")}</span><span style={{fontWeight:600}}>{qty.toLocaleString()}個／{pallets(qty,p).toFixed(1)}板</span></div>)})}</div>)}
                {sh.note&&(<div style={{fontSize:11,color:sh.status==="past"?"#94a3b8":"#64748b",background:sh.status==="past"?"#f8fafc":sh.note.includes("⚠️")?"#fef9c3":"#f0fdf4",borderRadius:8,padding:"7px 10px",marginTop:8}}>{sh.note}</div>)}
              </Card>
            );
          })}
          </>
        ...
        {activeTab==="alloc" && (
          ...
          <div style={{display:"flex",gap:8,overflowX:"auto",marginBottom:14,paddingBottom:2}}>
            {shipments.map((sh,si)=>{const p=shipmentPallets[si];const over=p>CONTAINER_PALLETS;return(<div key={sh.id} style={{background:"#fff",borderRadius:10,border:`1.5px solid ${over?"#fecaca":sh.status==="past"?"#f1f5f9":"#e2e8f0"}`,padding:"8px 12px",minWidth:80,flexShrink:0,textAlign:"center"}}><div style={{fontSize:9,fontWeight:700,color:"#64748b"}}>{sh.label}</div><div style={{fontSize:16,fontWeight:800,color:over?"#ef4444":sh.status==="past"?"#94a3b8":"#1e293b"}}>{p.toFixed(1)}</div><div style={{fontSize:9,color:"#94a3b8"}}>/{CONTAINER_PALLETS}板</div></div>);})}
          </div>
          {products.map(prod=>{
            ...
            return(
              <Card key={prod.id} borderColor={s.status==="danger"?"#fecaca":s.status==="warning"?"#fde68a":"#e2e8f0"}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                  <div style={{flex:1,marginRight:8}}><div style={{fontSize:13,fontWeight:700,lineHeight:1.4}}>{prod.name}</div><div style={{fontSize:10,color:"#94a3b8",fontFamily:"monospace"}}>{prod.id}</div></div>
                  <span style={{background:statusColors[s.status]+"22",color:statusColors[s.status],borderRadius:99,padding:"2px 8px",fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>{statusLabels[s.status]}</span>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:12,background:"#f8fafc",borderRadius:10,padding:"10px 12px"}}>
                  ...
                  <div><div style={{fontSize:9,color:"#94a3b8"}}>可撐天數</div><div style={{fontSize:13,fontWeight:700,color:statusColors[s.status]}}>{s.daysLeft>900?"—":Math.round(s.daysLeft)+"天"}</div></div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:6}}>
                  {shipments.map(sh=>{const qty=Number(sh.allocs[prod.id]||0);const pls=pallets(qty,prod);const isPast=sh.status==="past";return(<div key={sh.id}><div style={{fontSize:9,color:isPast?"#94a3b8":"#475569",fontWeight:600,marginBottom:3,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{sh.label}</div><input type="number" value={qty||""} placeholder="0" disabled={isPast} onChange={e=>updateAlloc(sh.id,prod.id,e.target.value)} style={{width:"100%",boxSizing:"border-box",border:`1.5px solid ${isPast?"#f1f5f9":qty>0?"#3b82f6":"#e2e8f0"}`,borderRadius:8,padding:"7px 5px",fontSize:12,textAlign:"right",background:isPast?"#f8fafc":qty>0?"#eff6ff":"#fff",fontWeight:600,color:isPast?"#94a3b8":"#1e293b"}}/><div style={{fontSize:9,color:"#94a3b8",textAlign:"right",marginTop:2}}>{qty>0?pls.toFixed(1)+"板":"—"}</div></div>);})}
                </div>
              </Card>
            );
          })}
        ...
        {activeTab==="suggest" && (
          ...
          <Card style={{background:"#eff6ff",borderColor:"#bfdbfe"}}>
            ...
            <div style={{fontSize:11,color:"#3730a3"}}>在途包含06/07已出貨但未到港的數量。</div>
          </Card>
          {suggestions.map(s=>(
            <Card key={s.id} borderColor={s.status==="danger"?"#fecaca":s.status==="warning"?"#fde68a":"#e2e8f0"}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                <div><div style={{fontSize:13,fontWeight:700}}>{s.name}</div><div style={{fontSize:10,color:"#94a3b8",fontFamily:"monospace"}}>{s.id}</div></div>
                <span style={{background:statusColors[s.status]+"22",color:statusColors[s.status],borderRadius:99,padding:"3px 10px",fontSize:11,fontWeight:700}}>{statusLabels[s.status]}</span>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px 20px",marginBottom:12}}>
                ...
                <div><Label>可撐天數</Label><BigNum color={statusColors[s.status]}>{s.daysLeft>900?"—":Math.round(s.daysLeft)+"天"}</BigNum></div>
                <div><Label>月均銷量</Label><BigNum>{Math.round(s.avg).toLocaleString()}</BigNum></div>
              </div>
              <div style={{background:"#f8fafc",borderRadius:10,padding:"10px 12px"}}><Label>各期已規劃</Label><div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:4}}>{shipments.map(sh=>{const qty=Number(sh.allocs[s.id]||0);if(qty===0)return null;return(<div key={sh.id} style={{background:sh.status==="past"?"#f1f5f9":"#eff6ff",borderRadius:8,padding:"4px 10px",fontSize:12}}><span style={{color:"#64748b"}}>{sh.label}：</span><span style={{fontWeight:700,color:sh.status==="past"?"#94a3b8":"#3b82f6"}}>{qty.toLocaleString()}</span></div>);})}</div></div>
            </Card>
          ))}
          </>
        )}
        ...
        {activeTab==="products" && (
          <>
          <div style={{fontSize:12,color:"#94a3b8",marginBottom:14}}>💡 10L=141包/板，20L=70包/板，50L=39包（占2板位）</div>
          {products.map((p,i)=>(
            <Card key={p.id}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                {[{label:"現有庫存",key:"stock",step:1,unit:"個"},{label:"每板數量",key:"upp",step:1,unit:"個/板"},{label:"佔板位數",key:"pslots",step:0.5,unit:"板位"},{label:"MOQ",key:"moq",step:1,unit:"個"}].map(({label,key,step,unit})=>(
                  <div key={key}><Label>{label}</Label><div style={{display:"flex",alignItems:"center",gap:6}}><input type="number" step={step} value={p[key]} onChange={e=>setProducts(prev=>prev.map((x,j)=>j===i?{...x,[key]:Number(e.target.value)}:x))} style={{flex:1,minWidth:0,border:"1px solid #e2e8f0",borderRadius:8,padding:"9px 10px",fontSize:15,textAlign:"right",fontWeight:600}}/><span style={{fontSize:11,color:"#94a3b8",whiteSpace:"nowrap"}}>{unit}</span></div></div>
                ))}
              </div>
            </Card>
          ))}
        ...
        {activeTab==="settings" && (
          ...
          <Card>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <input type="number" value={safetyDays} onChange={e=>setSafetyDays(Number(e.target.value))} style={{width:100,border:"1px solid #e2e8f0",borderRadius:8,padding:"10px 14px",fontSize:18,fontWeight:700}}/>
          ...
          <Card>
            ...
            <select value={avgMonths} onChange={e=>setAvgMonths(Number(e.target.value))} style={{width:"100%",border:"1px solid #e2e8f0",borderRadius:8,padding:"10px 14px",fontSize:16}}>
